...
namespace docopt {
	...
	long value::asLong() const
	{
		...
		if (kind == Kind::String) {
			...
			{
				const long ret = stol(str, &pos, 16); // Throws if it can't convert
				if (pos != str.length()) {
					// The string ended in non-hexdecimal digits.
					throw std::runtime_error( str + " contains non-hexidecimal characters.");
				}
				return ret;
			}
			else if ( str.size() > 1 && str[0] == '0' )
			{
				const long ret = stol(str, &pos, 8); // Throws if it can't convert
				if (pos != str.length()) {
					// The string ended in non-octal digits.
					throw std::runtime_error( str + " contains non-octal characters.");
				}
				return ret;
			}
			else
